...
describe('retrieve all module_examples', async () => {
  let app: Express
  beforeAll(async () => {
  ...
  it('retrieve success', async () => {
    ...
    await moduleExampleFactory.createMany(3)

    const moduleExamples = await DatabaseTestUtil.retrieveAll('module_examples')

    const response = await request(app).get(`/v1/transaction/module-examples`)
    ...
    expect(response.body.data[0].name).toStrictEqual(moduleExamples.data[0]['name'])
    expect(isValid(new Date(response.body.data[0].created_at))).toBeTruthy()
    expect(response.body.data[1].name).toStrictEqual(moduleExamples.data[1]['name'])
    expect(response.body.data[2].name).toStrictEqual(moduleExamples.data[2]['name'])
  ...
  it('sort data in ascending order', async () => {
    ...
    moduleExampleFactory.sequence(data)
    await moduleExampleFactory.createMany(3)

    const response = await request(app).get(`/v1/transaction/module-examples`).query({
      sort: 'name',
    })
  ...
  it('navigate pagination', async () => {
    ...
    expect(response.body.data[0].name).toStrictEqual(moduleExamples.data[2]['name'])
