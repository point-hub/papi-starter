...
const makeRouter = async ({ dbConnection }: IBaseAppInput) => {
  ...
  const routes: IRoute[] = [
    ...
    { method: 'get', path: '/:id/fields-summary', middlewares: [authMiddleware], controller: controller.fieldsSummaryController },
    { method: 'get', path: '/:id/field-histories', middlewares: [authMiddleware], controller: controller.fieldHistoriesController },
