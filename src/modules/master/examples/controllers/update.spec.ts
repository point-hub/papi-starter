...
describe('update an example', async () => {
  ...
  it('validate schema', async () => {
    ...
    const response = await request(app).patch(`/v1/master/examples/${resultFactory.inserted_id}`).send(updateData)
  ...
  it('update success', async () => {
    ...
    const response = await request(app).patch(`/v1/master/examples/${resultFactory.inserted_ids[1]}`).send(updateData)
    // expect http response
