...
export const refreshMiddleware: IMiddleware = async (middlewareInput: IMiddlewareInput) => {
  const bearer = middlewareInput.req['headers']['authorization'];
  const signedCookie = middlewareInput.req['cookies']['papp_starter_refresh'];

  const token = bearer && bearer.startsWith('Bearer ') ? bearer.split(' ')[1] : signedCookie;
  if (!token) { throwApiError(401); }

  const decoded = TokenService.verifyToken(token);
  if (!decoded) { throwApiError(401); }
